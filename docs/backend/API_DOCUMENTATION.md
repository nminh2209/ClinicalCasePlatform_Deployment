...
**Description:** Send password reset email to user with token link.
...
- Reset link format: `{FRONTEND_URL}/reset-password/{uid}/{token}/`
- No indication if email exists (security measure)
...
**Response (400 - Passwords Mismatch):**
...
**Description:** Login or register user via Google OAuth access token.
...
**Notes:**
- Verifies token with Google API: `https://www.googleapis.com/oauth2/v3/userinfo`
- Creates user automatically if doesn't exist (OAuth-only account)
- Sets unusable password (user must use Google login)
...
**Description:** Login or register user via Microsoft Azure AD access token.
...
**Notes:**
- Verifies token with Microsoft Graph API: `https://graph.microsoft.com/v1.0/me`
- Email extracted from `mail` or `userPrincipalName` field
- Creates user automatically if doesn't exist (OAuth-only account)
- Sets unusable password (user must use Microsoft login)
- Default role: "student"
