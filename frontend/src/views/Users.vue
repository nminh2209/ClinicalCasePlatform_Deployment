<template>
  <div class="users-page p-6 space-y-6">
    <!-- Header -->
    <div>
    ...
    >
      <div class="p-4 border-b border-gray-200">
        <h3 class="font-semibold text-gray-800">
          Danh sách người dùng ({{ filtered.length }} trên trang hiện tại)
        </h3>
      </div>
          ...
          <tbody>
            ...
            >
              ...
              <td class="px-6 py-4 text-gray-600">
                {{
                  user.department_vietnamese_name || user.department_name || "—"
                }}
              </td>
              <td class="px-6 py-4">
                <span
    ...
    >
      <div class="text-sm text-gray-600">
        Hiển thị <strong>{{ startIndex }}</strong> -
        <strong>{{ endIndex }}</strong> / <strong>{{ count }}</strong>
      </div>
      <div class="flex items-center space-x-2">
        ...
        <template v-for="p in pagesToShow" :key="p">
          <span v-if="p === '...'" class="px-3 py-1 text-sm">...</span>
          <Button
            v-else
            @click="gotoPage(p)"
            :label="String(p)"
...
const departmentFilter = ref(
  route.query.department ? String(route.query.department) : "",
);
...
const filtered = computed(() => {
  ...
  return users.value.filter((u) => {
    const fullName =
      ...
      "";
    const hay = normalize(
      `${fullName} ${u.email} ${u.department_name || ""} ${u.role}`,
    );
...
async function loadData(requestedPage: number = page.value) {
  ...
  try {
    ...
    users.value = results;
    count.value =
      typeof usersData.count === "number" ? usersData.count : results.length;
    ...
    router
      .replace({ query: { ...route.query, page: String(page.value) } })
      .catch(() => {});

    // Handle departments (should be array or paginated)
    const deptsData = deptsRes.data;
    const deptsList = deptsData.results
      ? deptsData.results
      : Array.isArray(deptsData)
        ? deptsData
        : [];
    departments.value = deptsList.map((d: any) => ({
      id: d.id,
      name: d.name,
...
const startIndex = computed(() =>
  count.value === 0 ? 0 : (page.value - 1) * pageSize.value + 1,
);
const endIndex = computed(() =>
  Math.min(count.value, (page.value - 1) * pageSize.value + users.value.length),
);
