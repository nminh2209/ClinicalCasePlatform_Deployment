...
def create_sample_image(title, description, width=800, height=600):
    ...
    for _ in range(200):
        ...
        draw.ellipse(
            [x, y, x + size, y + size], fill=(brightness, brightness, brightness)
        )
...
def create_sample_pdf(title, description, physician_notes, case_info):
    ...
    c.drawString(
        50,
        30,
        f"Tap tin mau duoc tao tu dong - {timezone.now().strftime('%d/%m/%Y %H:%M')}",
    )
        ...
        for i, attachment_data in enumerate(sample_attachments, 1):
            ...
            if attachment_data["file_type"] == "image":
                file_content = create_sample_image(
                    attachment_data["title"], attachment_data["description"]
                )
            ...
            print(f"   Type: {attachment.get_attachment_type_display()}")  # type: ignore[attr-defined]
