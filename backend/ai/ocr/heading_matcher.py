...
FIELD_MAPPINGS: Dict[str, List[str]] = {
    ...
    "clinical_history.social_history": ["Tiền sử xã hội", "Social history", "Lối sống"],
    ...
    "clinical_history.allergies": ["Dị ứng", "Allergies", "Tiền sử dị ứng"],
    ...
    "physical_examination.respiratory": ["Hô hấp", "Khám phổi", "Respiratory", "Phổi"],
    "physical_examination.abdominal": ["Bụng", "Khám bụng", "Abdominal", "Tiêu hóa"],
    "physical_examination.neurological": [
        "Thần kinh",
        "Khám thần kinh",
        "Neurological",
    ],
    "physical_examination.head_neck": ["Đầu cổ", "Khám đầu cổ", "Head and neck"],
    # Investigations
    ...
    "diagnosis_management.prognosis": ["Tiên lượng", "Prognosis"],
    # Case Summary
    "case_summary": ["Tóm tắt bệnh án", "Tóm tắt", "Case summary", "Summary"],
}
...
class HeadingMatcher:
    ...
    def _precompute_embeddings(self):
        ...
        # Encode all headings
        logger.info(
            f"Pre-computing embeddings for {len(self._heading_texts)} heading variations..."
        )
        self._field_embeddings = self._model.encode(
            self._heading_texts, convert_to_tensor=True, show_progress_bar=False
        )
    ...
    def match_heading(self, heading: str) -> Tuple[Optional[str], float]:
        ...
        heading_embedding = self._model.encode(
            heading, convert_to_tensor=True, show_progress_bar=False
        )
    ...
    def match_all_headings(self, headings: List[str]) -> Dict[str, Tuple[str, float]]:
        """
        ...
        heading_embeddings = self._model.encode(
            headings, convert_to_tensor=True, show_progress_bar=False
        )
...
def autofill_from_ocr(
    ocr_text: str, confidence_threshold: float = 0.6
) -> Dict[str, Any]:
    """
