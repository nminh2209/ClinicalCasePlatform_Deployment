...
def main():
    ...
    print(
        f"   GPU before: {mem_before['allocated']:.1f} MB allocated, {mem_before['reserved']:.1f} MB reserved"
    )
    print()
    ...
    try:
        ...
        print("=" * 60)
        print(
            f"⏱️  Time: {elapsed:.2f}s ({result.get('metadata', {}).get('elapsed_ms', 0)}ms reported)"
        )
