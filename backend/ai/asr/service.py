...
class ASRService:
    ...
    def _initialize_model(self):
        ...
        if not _ensure_asr_dependencies_loaded() or torch is None or pipeline is None:
            ...
            logger.info(
                "To enable ASR: pip install torch transformers accelerate soundfile librosa"
            )
            self._model = None
            return
        ...
        try:
            ...
            logger.info(
                "Downloading PhoWhisper model if not cached (this may take a few minutes)..."
            )
    ...
    def transcribe_bytes(
        self, audio_bytes: bytes, language: str = "vi", return_timestamps: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            result = self.transcribe(
                tmp_path, language=language, return_timestamps=return_timestamps
            )
