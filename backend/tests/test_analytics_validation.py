...
def test_analytics_system():
    ...
    for student in students:
        ...
        print(
            f"  ✓ {student.get_full_name()}: {metrics.total_cases_viewed} cases viewed"
        )
...
def test_validation_system():
    ...
    for rule in rules[:5]:
        print(f"    - {rule.name} ({rule.get_severity_display()})")  # type: ignore[attr-defined]
    ...
    for rule in rules:
        ...
        if not is_valid:
            validation_issues.append({"rule": rule.name, "message": error_message})
            rules_failed += 1
        else:
            rules_passed += 1
    ...
    print(f"  ✓ Workflow status: {workflow.get_current_status_display()}")  # type: ignore[attr-defined]
...
def test_integration():
    ...
    print(f"    ✓ Current status: {workflow.get_current_status_display()}")  # type: ignore[attr-defined]
...
def print_summary():
    ...
    print("\n✅ Validation:")
    print(
        f"  - Active Rules: {CaseValidationRule.objects.filter(is_active=True).count()}"
    )
