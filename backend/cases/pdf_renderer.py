...
class CaseTemplatePDFRenderer:
    ...
    def _add_header(self, canvas, doc):
        ...
        canvas.drawRightString(A4[0] - 2 * cm, A4[1] - 2 * cm, f"Trang {doc.page}")
    ...
    def render_template_structure(self):
        ...
        # Title
        title = Paragraph(f"<b>{self.template.name}</b>", self.styles["CustomTitle"])
        story.append(title)
        ...
        if self.template.attachment_requirements:
            ...
            story.append(
                Paragraph(
                    "<b>YÊU CẦU TÀI LIỆU ĐÍNH KÈM</b>", self.styles["SectionHeader"]
                )
            )
            ...
            for key, req in self.template.attachment_requirements.items():
                ...
                story.append(
                    Paragraph(
                        f"• {req.get('label', key)} {required}", self.styles["Normal"]
                    )
                )
    ...
    def _render_section(self, section):
        ...
        if fields:
            ...
            for field in fields:
                ...
                # Add field row
                field_data.append(
                    [f"{label}{required}{unit}", "____________________________________"]
                )
    ...
    def render_filled_case(self):
        ...
        # Title
        title = Paragraph(
            f"<b>BỆNH ÁN: {self.case.title}</b>", self.styles["CustomTitle"]
        )
        story.append(title)
        story.append(Spacer(1, 0.5 * cm))
        ...
        case_info = [
            ...
            [
                "Người tạo:",
                self.case.student.get_full_name() if self.case.student else "N/A",
            ],
    ...
    def _render_filled_section(self, section, case_data):
        ...
        if fields:
            ...
            for field in fields:
                ...
                field_data.append([f"{label}:", display_value])
...
def generate_case_pdf(case):
    ...
    patient_info = [
        ...
        [
            "Giới tính:",
            case.get_patient_gender_display() if case.patient_gender else "N/A",
        ],
    ...
    if case.admission_date:
        patient_info.append(
            ["Ngày nhập viện:", case.admission_date.strftime("%d/%m/%Y")]
        )
    if case.discharge_date:
        patient_info.append(
            ["Ngày xuất viện:", case.discharge_date.strftime("%d/%m/%Y")]
        )
    ...
    if hasattr(case, "clinical_history"):
        ...
        clinical_data = [
            ...
            [
                "Bệnh sử:",
                case.clinical_history.history_present_illness or "Chưa có dữ liệu",
            ],
            [
                "Tiền sử bệnh:",
                case.clinical_history.past_medical_history or "Chưa có dữ liệu",
            ],
            [
                "Tiền sử gia đình:",
                case.clinical_history.family_history or "Chưa có dữ liệu",
            ],
            [
                "Tiền sử xã hội:",
                case.clinical_history.social_history or "Chưa có dữ liệu",
            ],
            ["Dị ứng:", case.clinical_history.allergies or "Chưa có dữ liệu"],
            [
                "Thuốc đang dùng:",
                case.clinical_history.medications or "Chưa có dữ liệu",
            ],
        ]
    else:
        story.append(Paragraph("<b>TIỀN SỬ BỆNH</b>", section_style))
    ...
    if hasattr(case, "physical_examination"):
        ...
        exam_data = [
            [
                "Tình trạng chung:",
                case.physical_examination.general_appearance or "Chưa có dữ liệu",
            ],
            [
                "Dấu hiệu sinh tồn:",
                case.physical_examination.vital_signs or "Chưa có dữ liệu",
            ],
            ["Đầu - Cổ:", case.physical_examination.head_neck or "Chưa có dữ liệu"],
            [
                "Tim mạch:",
                case.physical_examination.cardiovascular or "Chưa có dữ liệu",
            ],
            ...
            [
                "Cơ xương khớp:",
                case.physical_examination.musculoskeletal or "Chưa có dữ liệu",
            ],
    ...
    if hasattr(case, "diagnosis_management"):
        ...
        diagnosis_data = [
            [
                "Chẩn đoán chính:",
                case.diagnosis_management.primary_diagnosis or "Chưa có dữ liệu",
            ],
            [
                "Chẩn đoán phân biệt:",
                case.diagnosis_management.differential_diagnosis or "Chưa có dữ liệu",
            ],
            [
                "Kế hoạch điều trị:",
                case.diagnosis_management.treatment_plan or "Chưa có dữ liệu",
            ],
            [
                "Thuốc điều trị:",
                case.diagnosis_management.medications_prescribed or "Chưa có dữ liệu",
            ],
            [
                "Thủ thuật:",
                case.diagnosis_management.procedures_performed or "Chưa có dữ liệu",
            ],
            ["Tiên lượng:", case.diagnosis_management.prognosis or "Chưa có dữ liệu"],
            [
                "Theo dõi:",
                case.diagnosis_management.follow_up_plan or "Chưa có dữ liệu",
            ],
        ]
    else:
        story.append(Paragraph("<b>CHẨN ĐOÁN VÀ ĐIỀU TRỊ</b>", section_style))
    ...
    if attachments.exists():
        for idx, att in enumerate(attachments, 1):
            ...
            uploader_name = (
                att.uploaded_by.get_full_name() if att.uploaded_by else "N/A"
            )
            ...
            try:
                ...
                if file_ext in ["jpg", "jpeg", "png", "gif"]:
                    ...
                    try:
                        ...
                        images = convert_from_path(
                            file_path, dpi=150, first_page=1, last_page=5
                        )  # Limit to 5 pages
                        ...
                        for page_num, pil_img in enumerate(images, 1):
                            ...
                            page_label = Paragraph(
                                f"<i>Trang {page_num}:</i>", normal_style
                            )
                ...
                note = Paragraph(
                    f"<i>Không thể hiển thị file đính kèm</i>", normal_style
                )
                story.append(note)
                story.append(Spacer(1, 0.3 * cm))
    else:
        no_attachment_table = Table([["Không có file đính kèm"]], colWidths=[16 * cm])
        no_attachment_table.setStyle(
            TableStyle(
                [
                    ("FONT", (0, 0), (-1, -1), base_font, 10),
