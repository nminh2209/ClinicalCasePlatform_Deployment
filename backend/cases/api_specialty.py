        ...
        return Response(
            {
                "specialties": SpecialtySerializer(
                    Specialty.objects.filter(is_active=True), many=True
                ).data,
                "priorities": PriorityLevelSerializer(
                    CasePriorityLevel.objects.filter(is_active=True), many=True
                ).data,
                "complexities": ComplexityLevelSerializer(
                    CaseComplexityLevel.objects.filter(is_active=True), many=True
                ).data,
            }
        )
