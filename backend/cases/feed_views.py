...
class PublicFeedListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        filter_type = self.request.query_params.get("filter", "department")  # type: ignore[attr-defined]

        if filter_type == "department" and user.department:  # type: ignore[attr-defined]
            queryset = queryset.filter(
                Q(feed_visibility="department", student__department=user.department)  # type: ignore[attr-defined]
...
class PublicFeedDetailView(generics.RetrieveAPIView):
    ...
    def get_queryset(self):
        return Case.objects.filter(
            is_published_to_feed=True, case_status=Case.StatusChoices.APPROVED
        ).select_related("student", "student__department", "published_by", "repository")
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        user_reaction = Comment.objects.filter(
            case=instance, author=request.user, is_reaction=True
        ).first()
...
def unpublish_from_feed(request, pk):
    ...
    return Response(
        {"message": "Case unpublished from feed successfully", "case_id": case.id}  # type: ignore[attr-defined]
    )
...
def get_case_reactions(request, pk):
    ...
    recent_reactions = (
        Comment.objects.filter(case=case, is_reaction=True)
...
def feed_statistics(request):
    ...
    stats = {
        ...
        "recent_featured": queryset.filter(is_featured=True)
        .order_by("-published_to_feed_at")[:3]
        .values("id", "title", "specialty", "published_to_feed_at"),
    }
