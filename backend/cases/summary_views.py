    ...
    try:
        ...
        try:
            if user.is_student:
                ...
                # Instructors should not see/count draft cases
        ...
        complexity_stats = (
            queryset.values("complexity_level")
            .annotate(count=Count("id"))
        ...
        department_stats = (
            queryset.values(
                "student__department__name", "student__department__vietnamese_name"
            )
            .annotate(count=Count("id"))
            .order_by("-count")
        )
        ...
        daily_stats = (
            recent_cases.extra(select={"day": "DATE(cases_case.created_at)"})
            .values("day")
            .annotate(count=Count("id"))
            .order_by("day")
        )
        ...
        if queryset.filter(grade__isnull=False).exists():
            ...
            avg_grades = grades_queryset.aggregate(
                avg_score=Avg("score"), total_graded=Count("id")
            )
...
def case_summary_list(request):
    ...
    queryset = Case.objects.select_related(
        "student", "student__department", "repository"
    ...
    response_data = {"count": queryset.count(), "cases": serializer.data}
...
def export_case_summary(request):
    ...
    # Build queryset
    queryset = Case.objects.select_related("student", "student__department")
    ...
    for key, value in filters.items():
        if value:
            if key == "department":
                queryset = queryset.filter(
                    Q(student__department_id=value) | Q(repository__department_id=value)
                )
    ...
    if export_format == "json":
        serializer = (
            CaseDetailSerializer(queryset, many=True) if include_details else None
        )
        ...
        if include_statistics:
            ...
            stats_request = type(
                "obj", (object,), {"user": user, "query_params": filters}
            )()
            # Add statistics here (simplified for now)
            response_data["statistics"] = {
                "total_cases": queryset.count(),
                "by_status": list(
                    queryset.values("case_status").annotate(count=Count("id"))
                ),
            }
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="case_summary_{timezone.now().strftime("%Y%m%d_%H%M%S")}.csv"'
        )
